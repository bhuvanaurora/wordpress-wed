...
class WebnusWidgetTabs extends WP_Widget{
	...
	function __construct(){
		...
		$params = array(
		'description'=> 'Webnus Widget Tabs',
		'name'=> 'Webnus-Widget-Tabs'
		);
	...
	public function form($instance){
		...
		extract($instance);
		$defaults = array('posts' => 5, 'comments' => '5', 'tags' => 5, 'show_popular_posts' => 'on', 'show_recent_posts' => 'on', 'show_comments' => 'on', 'show_tags' =>  'on', 'orderby' => 'Highest Comments');
		...
		<p>
			...
			<select id="<?php echo $this->get_field_id('orderby'); ?>" name="<?php echo $this->get_field_name('orderby'); ?>" class="widefat" style="width:100%;">
				<option <?php if ('Highest Comments' == $instance['orderby']) echo 'selected="selected"'; ?>>Comments</option>
				<option <?php if ('Highest Views' == $instance['orderby']) echo 'selected="selected"'; ?>>Views</option>
			</select>
		</p>
		<p>
					...
					<div id="tab-popular" class="tab tab_content" style="display: none;">
						...
						<ul class="tab-list">
							...
							<li>
								...
								<?php endif; ?>
								<div class="content">
									<a class="tab-title" href="<?php the_permalink(); ?>"><?php the_title(); ?></a>
									<div class="tab-meta"><strong><?php _e('by ','WEBNUS_TEXT_DOMAIN'); ?></strong><?php the_author_link(); ?> <strong><?php _e('in ','WEBNUS_TEXT_DOMAIN'); ?></strong><?php the_category(' / ') ?></div>
									<div class="tab-meta">
										<span class="tab-date"><i class="fa-clock-o"></i> <?php the_time('d M y'); ?></span>	
										<span class="tab-views"> <i class="fa-eye"></i> <?php echo getViews(get_the_ID()); ?></span>
										<span class="tab-comments"> <i class="fa-comment-o"></i> <?php comments_number( '0', '1', '%' ); ?> </span>
									</div>
								</div>
							</li>
							<?php endwhile; ?>
						...
						<ul class="tab-list">
							...
							<li>
								...
								<div class="content">
									...
									<div>
										<a class="comment-text-side" href="<?php echo get_permalink($comment->ID); ?>#comment-<?php echo $comment->comment_ID; ?>" title="<?php echo strip_tags($comment->comment_author); ?> on <?php echo $comment->post_title; ?>"><?php echo strip_tags($comment->com_excerpt); ?>...</a>
									</div>
								</div>
							</li>
							<?php } ?>
